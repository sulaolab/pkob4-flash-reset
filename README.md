...
- `reset_pkob4` defaults to a 60 second per-attempt timeout: a healthy boost returns
  in ~2-3s and the tool exits as soon as it does, so the high cap is essentially free,
  while a COLD boost connect (first run, right after `--clean-java`, or a just-
  re-enumerated PKOB4) routinely needs well over 15s. Pass `--timeout` for a faster
  fail if desired.
...
cold start を見込んで既定 timeout は reset / probe とも 60 秒です（成功時は即 return するため上限は実質無コスト）。
